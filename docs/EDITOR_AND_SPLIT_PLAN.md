...
> **2026-08-25 改訂**: Phase B の仕様は `docs/MULTI_PANE_PROPOSAL.md`（Context Mixer `17_マルチペインと注意の観測`）で**フラットな可変ペイン**に改訂済み。以下の旧仕様（2分割固定）は経緯として保持。改訂の主な変更点: ①2分割固定 → 可変ペイン配列（入れ子は禁止・ソフト上限 4）②プレビュー/ブラウザも任意ペインへ配置可（webview 再読込は許容）③get_focus v2 は Phase 8 に含めない（D13 再々改訂の未決のため。代わりに push 拡張 §4.7 を含む）④ドラッグ&ドロップは含めない（右クリック移動のみ）⑤見積もり 6〜7日
...
> **2026-08-25 改訂**: Phase 8（L2）は 2026-08-25 に完了（B0〜B6）。`get_focus` v2 は Phase 8 に含めず、**D13 再々改訂として別に扱う**（`docs/MULTI_PANE_PROPOSAL.md` §5 参照）。以下は §5 の2条件へ差し替え。

着手は別途判断する。**Phase 8 完了後に、以下の2点を決めてから `get_focus` v2 に着手する**:

1. **呼び出し元の識別と認証** — D13 再改訂が `http://127.0.0.1:41001/mcp` 固定・無認証へ移行した正当化は「公開する操作が `show_file`（人間に見せるだけ）に限られるため」が条件付きだった。`get_focus` v2 は pull 系そのものであり、無認証の固定 URL では「別プロジェクトのエージェントへ人間の注意を返す」が既定動作になる。2026-08-25 時点で無認証・固定 URL 化そのものが未実装のため、選択肢はまだ閉じていない
2. **スナップショットか遷移か** — D24（`docs/MULTI_PANE_PROPOSAL.md` §3）が示唆する価値の中心が**遷移**（エージェントの出力が出た数秒後にドキュメントペインへ目が移った、等）にあるなら、必要なのは要求応答の `get_focus` ではなく**イベントログとその読み出し**である。この選択が決まらないうちにスキーマを固定すると後で作り直すことになる

**push 拡張（Phase 8 B4 で実装済み）が D24 の最小検証を兼ねる**。送信時に可視ペイン構成を混ぜてエージェントの応答が変わらなければ、pull へ投資する理由はない（§6）。

**その他の回収項目**:

- `pending_marks`（D13-8）— CM6 のガターに印を付ける。Phase A の上に乗る。「非同期の印」モデルのため、ブロッキングにしないこと
- `get_focus` の再検討 — 上記の2条件を決めた後に着手。ターミナルとファイルが同時に見える状態が初めて生まれるため、`watching_you` と `file` が両立する。スキーマは単数の `file` / `terminal` ではなく**ペイン集合 + `hasFocus`** を基本形とする（D24）
